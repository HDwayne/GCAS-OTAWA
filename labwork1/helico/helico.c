...
//     return 0;
// }
// 15:10:09 dwayne@dynabook ~/Documents/otawa/labwork1/helico $ owcet -s lpc2138 helico.elf updateADC --stats -W
// INFO: plugged otawa/lpc2138 (/home/dwayne/Documents/otawa/otawa/lib/otawa/otawa/lpc2138.so)
// WARNING: otawa::FlowFactLoader 1.4.0:loop exists at 00000574 but no bound at helico.ffx:128
// WARNING: otawa::FlowFactLoader 1.4.0:loop exists at 0000062c but no bound at helico.ffx:130
...
void stabilize() {
  Ainlast = Ain;
  Ain=getValue(GYRO_CHANNEL);
  doGyroChannel();
  action();
  doPWM();

  Ainlast = Ain;
  Ain=getValue(AROMX_CHANNEL);
  doAROMXChannel();
  action();
  doPWM();

  Ainlast = Ain;
  Ain=getValue(AROMY_CHANNEL);
  doAROMYChannel();
  action();
  doPWM();

  Ainlast = Ain;
  Ain=getValue(AROMZ_CHANNEL);
  doAROMZChannel();
  action();
  doPWM();	
}
